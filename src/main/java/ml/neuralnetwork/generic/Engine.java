...
import ml.neuralnetwork.ActivatationFunctions;
import ml.neuralnetwork.Utils;
...
public class Engine {
    ...
    public static Double[] process(TrainingSet<Double, Double> input) {
        //logger.info("processing " + input);
        Double[] weights = new Double[input.getSample(0).getSampleSize()];
        Arrays.fill(weights, new Double(0.5));
        logger.info("initial weights " + Arrays.asList(weights));
//        for (TrainingSet.TrainingSample<Double, Double> sample : input.getSamples()) {
        for (TrainingSet.TrainingSample sample : input.getSamples()) {
            logger.info("processing : " + sample);
            logger.info("weighted input : " + inputCombiner.apply((Double[])sample.getInput(), weights));
            logger.info("output : " + activationFunction.apply(inputCombiner.apply((Double[])sample.getInput(), weights)));
            logger.info("target output : " + (Double)sample.getOutput());
            weights = calculateNewWeights.
                    apply(learningRate).
                    apply(residualError.apply(
                            activationFunction.apply(inputCombiner.apply((Double[])sample.getInput(), weights)),
                            (Double)sample.getOutput()
                    )).
                    apply((Double[])sample.getInput(), weights);
